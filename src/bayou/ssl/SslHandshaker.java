...
class SslHandshaker
{
    ...
    SSLEngine engine;
    Promise<SslConnection> promise;

    SslHandshaker(TcpChannel channel, Promise<SslConnection> promise, long id, _ByteBufferPool bufferPool, SSLEngine engine)
    {
    ...
    int read() throws Exception
    {
        ...
        if(r==-1) // premature tcp close
            return kill(new IOException("peer closed the connection during SSL handshake"));
    ...
    int unwrap() throws Exception
    {
        ...
        switch(result.getHandshakeStatus())
        {
            ...
            case FINISHED: // possible in resumed handshake, where client sends the last handshake msg.
                // note: readBuffer!=null. may or may not contain data. finish() will deal with it.
                if(trace)trace("free appBuffer4Unwrap");
    ...
    int finish()
    {
        ...
        SslConnectionImpl sslConn = new SslConnectionImpl(channel, engine, bufferPool, id);
        if(readBuffer!=null)
        {
            if(readBuffer.hasRemaining())
            {
            ...
            {
                // we could handover the empty buffer to sslConn, set needMorePeerBytes=true.
    ...
    void asyncRunTasks(int jNext)
    {
        Async<Void> asyncExec = Async.execute((Callable_Void)() ->
        {
            Runnable task;
        ...
        promise.onCancel(asyncExec::cancel);

        asyncExec.onCompletion( (Result<Void> result) ->
        {
            Exception error = result.getException();
            if (trace) trace("tasks done");
            if (error != null)
                kill(error);
            else
                jump(jNext);
